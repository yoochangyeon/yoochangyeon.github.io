...
category: "Backend"
tags: ["nestjs", "caching", "performance"]
...
직접 만들면 손이 많이 가지만, Redis가 죽었을 때 어떻게 동작하는지, TTL이 동시에 만료되면 어떤 일이 생기는지 전부 코드로 보이니까 팀원한테 설명하기가 편합니다. 외부 패키지 안에서 무슨 일이 일어나는지 뒤지는 시간을 생각하면, 오히려 이쪽이 유지보수가 수월한 것 같습니다.
