...
author: "유창연"
title: "HTTP 캐시 헤더 정리: Cache-Control부터 304 Not Modified까지"
excerpt: "브라우저와 CDN 캐싱을 제어하는 HTTP 헤더를 정리합니다. Cache-Control, ETag, 304 Not Modified의 동작 원리와 CloudFront에서의 캐싱 전략을 설명합니다."
description: "Cache-Control, ETag, Last-Modified 헤더와 304 Not Modified 응답의 동작 원리를 정리합니다. 브라우저 캐시와 CDN(CloudFront) 캐싱 전략을 예제와 함께 설명합니다."
...
웹 성능 최적화에서 캐싱은 효과가 큰 방법 중 하나입니다. 적절한 캐시 전략을 세우면 서버 부하와 네트워크 트래픽이 줄어들고, 페이지 로딩이 빨라집니다.

다만 HTTP 캐싱은 생각보다 복잡합니다. 브라우저 캐시, CDN 캐시, 조건부 요청, 재검증 등 다양한 개념이 얽혀 있고, 각각의 HTTP 헤더가 어떻게 상호작용하는지 이해해야 제대로 활용할 수 있습니다.

이 글에서는 HTTP 캐싱 관련 헤더들을 정리합니다.
...
리소스 유형별로 캐시 전략을 다르게 가져가는 것이 기본입니다.

- 정적 자산(JS, CSS, 이미지): `Cache-Control: public, max-age=31536000, immutable`로 장기 캐싱하고, 파일명에 해시를 포함해서 버전 관리
- HTML 페이지: `Cache-Control: public, max-age=0, must-revalidate`로 항상 재검증하되, 304 응답으로 트래픽 절감
- API 응답: `max-age`와 `stale-while-revalidate`를 조합해서 신선도와 응답 속도의 균형 맞추기
- 민감한 데이터: `no-store, private`으로 캐싱 자체를 차단

CloudFront를 사용한다면 `s-maxage`로 CDN 캐시와 브라우저 캐시를 분리하고, Minimum TTL이 0보다 크면 `no-cache`가 무시된다는 점을 주의해야 합니다.

캐시가 제대로 동작하지 않을 때는 라우팅 순서대로 확인하면 됩니다. Cache-Control 헤더가 올바른지, CloudFront TTL이 오버라이드하고 있지 않은지, ETag/Last-Modified가 정상적으로 생성되는지 하나씩 따라가면 대부분 원인을 찾을 수 있습니다.
